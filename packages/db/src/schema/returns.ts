...
export const returnCondition = pgEnum('return_condition', ['UNCLEARED_PAYMENT', 'DEFECTIVE']);
...
export const returns = pgTable(
  ...
  (t) => [
    ...
    check('returns_split', sql`${t.amount} = ${t.toSale} + ${t.toOtherDebts} + ${t.refund} and ${t.toSale} >= 0 and ${t.toOtherDebts} >= 0 and ${t.refund} >= 0`),
    check('returns_collected', sql`${t.collectedPortion} = ${t.toOtherDebts} + ${t.refund}`),
